			              	 ...
			              	 return (
										<div className="Container">				
											<div className="Box">
												...
												{item.phone &&  
													<h3>Τηλέφωνο: {item.phone}</h3>
												}
												{item.phone2 && 
													<h3>Τηλέφωνο: {item.phone2}</h3>
												}
												{item.bank && 
													<h3>{item.bank}: {item.IBAN}</h3>
												}
												{item.bank2 && 
													<h3>{item.bank2}: {item.IBAN2}</h3>
												}
												{item.bank3 && 
													<h3>{item.bank3}: {item.IBAN3}</h3>
												}
												<div>
														<Button className='deleteIcon' onClick={() => deletePartner(item.name, item.id)}>
															<DeleteIcon  fontSize="large"/>
														</Button>

														<div className="editIcon">
															<CustomizedDialogsEdit >
			                              <EditPartnerForm  getPartnersData={getPartnersData} {...item}/>
								               </CustomizedDialogsEdit>
						            		</div>
					              </div>
